...
public class OxygenTank extends Collectable{
    ...
    public void collect() {
        ...
        if(GameManager.gameManager.currentOxygen > 1000)
        {
            //add to score by bonus "overflow"
            GameManager.gameManager.baseScore += (int) ((double) (GameManager.gameManager.currentOxygen - 1000) /40);
        }
        //Reduced the oxygen tank replenish amount to 25% as 75%  was too much
        GameManager.gameManager.currentOxygen = PApplet.min(4000, GameManager.gameManager.currentOxygen + 1000);
    }
}